...
#[ApiEntity(\App\Entity\Treatment::class)]
    ...
    public function create(Request $request): Response
    {
        try {
            ...
            if (!empty($errors['entity'])) {
                ...
                $response = $this->serializer->serialize($errors['entity'], 'json', ['groups' => 'treatment:read']);
                $response = json_decode($response, true);
                return $this->json(['data' => $response,'code' => 200, 'message' => "Treatment crée avec succès"], Response::HTTP_OK);
            }
    ...
    public function update(Request $request,  $id): Response
    {
        try {
            ...
            if (!empty($errors['entity'])) {
                ...
                $response = $this->serializer->serialize($errors['entity'], 'json', ['groups' => 'treatment:read']);
                $response = json_decode($response, true);
                return $this->json(['data' => $response,'code' => 200, 'message' => "Treatment modifié avec succès"], Response::HTTP_OK);
            }
