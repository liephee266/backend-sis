...
#[ApiEntity(\App\Entity\Examination::class)]
    ...
    public function create(Request $request): Response
    {
        try {
            ...
            if (!empty($errors['entity'])) {
                ...
                $response = $this->serializer->serialize($errors['entity'], 'json', ['groups' => 'examination:read']);
                $response = json_decode($response, true);
                return $this->json(['data' => $response,'code' => 200, 'message' => "Examination crée avec succès"], Response::HTTP_OK);
            }
    ...
    public function update(Request $request,  $id): Response
    {
        try {
            ...
            if (!empty($errors['entity'])) {
                ...
                $response = $this->serializer->serialize($errors['entity'], 'json', ['groups' => 'examination:read']);
                $response = json_decode($response, true);
                return $this->json(["data"=> $response, 'code' => 200, 'message' => "Examination modifié avec succès"], Response::HTTP_OK);
            }
